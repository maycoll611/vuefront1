<template>
  <v-col>
    <v-row class="justify-center">
      <v-col md="3" sm="8">
        <v-form @submit.prevent="iniciar()" ref="form_login" >
          <v-card class="rounded" elevation="8">
            <v-col class="pb-0" align="center">
                <v-alert color="teal lighten-2">Logistica GR1</v-alert>
             <!-- <v-img src="logotipo.png" width="300"></v-img> -->
            </v-col>
            <v-col class="pb-0">
              <v-text-field outlined dense required="required" color="teal darken-1" v-model="usuario.usuario_nombre" label="Nombre de usuario" ></v-text-field>
            </v-col>
            <v-col class="py-0">
              <v-text-field outlined dense required="required" autocomplete="on" color="teal darken-1" v-model="usuario.usuario_pw" type="password" label="password"></v-text-field>
            </v-col>
            <v-col align="end" class="mb-5">
              <v-btn @click="prueba" color="red lighten-1" text-color="white" > prueba</v-btn>
              <v-btn type="submit" color="teal lighten-1" text-color="white" > Entrar</v-btn>
            </v-col>
            <v-col>
